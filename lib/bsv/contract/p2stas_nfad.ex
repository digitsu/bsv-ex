defmodule BSV.Contract.P2STAS_NFAD do
  ...
  * `:keypair` - A `t:BSV.KeyPair.t/0` struct.
  ...
  ## Examples
      ...
      iex> contract = P2STAS.unlock(%UTXO{}, %{keypair: @keypair})
      iex> Contract.to_script(contract)
      %Script{chunks: [
  ...
  def op_stas(ctx) do
    asm = BSV.Script.from_asm!(@op_stasnftad)
    Enum.reduce(Map.get(asm, :chunks), ctx, &asm_push/2)
  end

  def op_check_tx(ctx) do
    asm = BSV.Script.from_asm!(@op_check_tx)
    Enum.reduce(Map.get(asm, :chunks), ctx, &asm_push/2)
  end
  ...
  def suffix(ctx, redeem_address, data, flags \\ <<>>) do
  ...
  def locking_script(ctx, %{
    ...
    |> op_check_tx()
    |> op_stas()
    |> suffix(redeem_address, data, flags)
  end

  @impl true
  def unlocking_script(ctx, %{address: address, keypair: keypair}) do
    ctx
    # stas always just have 1 sat
    |> 1
    |> push(address.pubkey_hash)
    |> "data"
    |> op_false
